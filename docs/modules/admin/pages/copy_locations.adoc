...
+
.. _Check In Alert_, choose whether you would like a routing alert to appear when an item in this location is checked in. This is intended for special locations, such as 'Display', that may require special handling, or that temporarily contain items that are not normally in that location.
+
...
. If it is configured to display in the public catalog, the shelving location group will also be visible in the staff catalog.
It will appear in the Library selector and/or the Library Groups selector.  You can configure where it appears with the
_staff.search.shelving_location_groups_with_orgs_ (for Library selector) and the
_staff.search.shelving_location_groups_with_lassos_ (for Library Groups selector) global flags.
