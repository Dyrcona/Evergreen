...
image::reporter_basics/reports-filters-1.png["Screenshot of a check in Scan date/time date filter. Operator of between, filter values selected for the first and last dat of 2023. Transform is date. Highlighted in the lower left is a hint, enter the earlier date in the first box."]
...
image::reporter_basics/reports-filters-2.png[Screenshot of a check in scan date/time. The relative time value box is selected. Instead of date fields with a calendar pickers there are fields for number of days ago.]
